...
int command_line::parser(int ac, char* av[])
{
  ...
  run_desc.add_options()
	("help,h", "Show help")
    ("input,i", po::value<std::string>(&input_path)->default_value("E:/project/input.json",""), "Input .json file")
    ("config,c", po::value<std::string>(&config_path)->default_value("E:/project/config.ini",""), "Input .ini config file")
    ("parametrs,p", po::value<std::vector<std::string>>()->multitoken()->notifier(vector_notifer), "Output parameters separated by spaces")
    ("output,o", po::value<std::string>(&output_path)->default_value("E:/project/output.txt",""), "Output parameters file .txt")
